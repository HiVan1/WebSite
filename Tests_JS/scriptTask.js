...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function formSend(event){
        ...
        if (er === 0){
            alert('Результаты отправлены на почту');
    ...
    function sendMail(time){
        Email.send({
            SecureToken:"846186c3-71bc-437b-9570-0fb5a4f422ef",
            
            To : document.getElementById("mail").value,
            From : "kiohi2000@gmail.com",
            Subject : "Результаты теста на сайте 'Test JS'",
            Body : "<h1>Привет "+student_user.Name+"!</h1>"+
            "<h3>Короткий отчет по тесту:</h3>"+
            "<br>Пользователь: " + student_user.Name +
            "<br>Получено балов: " + student_user.Point +
            "<br>Время начала: " + time +
            "<br>Время сдачи: " + date.toISOString() +
            "<h3>Если тебе понравился наш тест, мы будем рады, если ты поделишься этим тестом с друзьями чтобы они тоже смогли проверить свои знания</h3>"
        }).then(
